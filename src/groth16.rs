...
pub fn prove_linked_g16_equations<R, E>(g16_pf_elems: &Vec<(&Proof<E>, &VerifyingKey<E>, &E::G1Affine)>, g16_linked_inputs: &Vec<E::Fr>, gs_crs: &GS_CRS<E>, rng: &mut R) -> (Commit1<E>, Commit2<E>, Vec<EquProof<E>>)
where
    R: Rng + CryptoRng,
    E: PairingEngine
{
    ...
    let m = 3 * num_equ;
    let mut xvars: Vec<E::G1Affine> = Vec::with_capacity(m);
    for (_, (pf, vk, _)) in g16_pf_elems.iter().enumerate() {

        assert!(g16_linked_inputs.len() <= vk.gamma_abc_g1.len());

        // Compute sum from i=0 to k-1 of a_p,i W_p,i
        let mut s_link_var = E::G1Affine::zero();
        for (i, ai) in g16_linked_inputs.iter().enumerate() {
            s_link_var = s_link_var + vk.gamma_abc_g1[i].mul(*ai).into_affine();
        }
        xvars.append(&mut vec![pf.a, pf.c, s_link_var]);
    }
    // Y = [ ..., B_p, ... ]
    let _n = num_equ;
    let yvars: Vec<E::G2Affine> = g16_pf_elems.iter()
...
pub fn prepare_linked_g16_equations<E: PairingEngine>(g16_elems: &Vec<(&VerifyingKey<E>, &E::G1Affine)>) -> Vec<PPE<E>> {
...
//    type G2Affine = <F as PairingEngine>::G2Affine;
    ...
    fn test_mock_GS_over_linked_Groth16_verification() {
        ...
        let (xcoms, ycoms, gs_proofs) = prove_linked_g16_equations(&vec![(&mock_g16_pf, &mock_g16_vk, &mock_prepared_public_input)], &vec![mock_a0, -mock_a0], &crs, &mut rng);
        assert!(verify_linked_g16_equations::<F>(&vec![(&mock_g16_vk, &mock_prepared_public_input)], (&xcoms, &ycoms, &gs_proofs), &crs));
    }
    ...
    fn test_mock_GS_over_linked_Groth16_multi_verification() {
        ...
        let mock_a0 = Fr::rand(&mut rng);
        let (xcoms, ycoms, gs_proofs) = prove_linked_g16_equations(&vec![
              (&mock_g16_pf1, &mock_g16_vk1, &mock_prepared_public_input1),
              (&mock_g16_pf2, &mock_g16_vk2, &mock_prepared_public_input2),
              (&mock_g16_pf3, &mock_g16_vk3, &mock_prepared_public_input3)
        ], &vec![mock_a0, -mock_a0], &crs, &mut rng);
        assert!(verify_linked_g16_equations::<F>(&vec![
                (&mock_g16_vk1, &mock_prepared_public_input1),
                (&mock_g16_vk2, &mock_prepared_public_input2),
                (&mock_g16_vk3, &mock_prepared_public_input3)
        ], (&xcoms, &ycoms, &gs_proofs), &crs));
