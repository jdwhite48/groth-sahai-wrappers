...
fn bench_GS_over_Groth16_one_prove(c: &mut Criterion) {
    ...
    c.bench_function(
        &format!("Prove satisfiability of 1 Groth16 verification equation using Groth-Sahai"),
        |bench| {
            bench.iter(|| {

                let _ = prove_canon_g16_equations(&vec![(&mock_g16_pf, &mock_g16_vk, &mock_prepared_public_input)], &crs, &mut rng);
            });
        }
    );
}
...
fn bench_GS_over_Groth16_one_verify(c: &mut Criterion) {
    ...
    c.bench_function(
        &format!("Verify satisfiability of 1 Groth16 verification equation using Groth-Sahai"),
        |bench| {
            bench.iter(|| {
                let _ = verify_canon_g16_equations::<F>(&vec![(&mock_g16_vk, &mock_prepared_public_input)], (&xcoms, &ycoms, &gs_proofs), &crs);
            });
        }
    );
}
...
fn bench_GS_over_Groth16_three_prove(c: &mut Criterion) {
    ...
    c.bench_function(
        &format!("Prove satisfiability of 3 Groth16 verification equations using Groth-Sahai"),
        |bench| {
            bench.iter(|| {

                let _ = prove_canon_g16_equations(&vec![
...
fn bench_GS_over_Groth16_three_verify(c: &mut Criterion) {
    ...
    c.bench_function(
        &format!("Verify satisfiability of 3 Groth16 verification equations using Groth-Sahai"),
        |bench| {
            bench.iter(|| {
                let _ = verify_canon_g16_equations::<F>(&vec![
